  ...
  constructor() {
    ...
    this.chooseclass.onclick = (_) => {
      ...
      classchoose.setDisplay(
        "Choose class",
        `<center>Your classrooms</center>${viewbody}`
      );
      if (this.data.pseudoclasses.length) {
        let pclassbtns = [];
        this.data.pseudoclasses.forEach((pClass, pc) => {
                ...
                postJsonData(post.student.classroom, {
                  ...
                  .then((response) => {
                    ...
                    switch (response.event) {
                      ...
                      default:
                        return parent.snackbar(response.event);
                    }
                  })
                  .catch((e) => {
                    parent.snackbar(e);
    ...
    this.joinclass.onclick = (_) => {
      ...
      reqdialog.setDisplay(
        "Type classname",
        "Provide the classname to request membership"
      );
      reqdialog.createInputs(
        ["Valid classname"],
        ["A classname of your institute"],
        ["text"],
        [validType.nonempty]
      );
      reqdialog.createActions(
        ["Send Request", "Cancel"],
        [actionType.positive, actionType.neutral]
      );
      reqdialog.validate();
      reqdialog.transparent();
      reqdialog.onButtonClick([
        (_) => {
          ...
          postJsonData(post.student.classroom, {
            ...
            .then((response) => {
              if (response.event == code.OK) {
                reqdialog.hide();
                return parent.snackbar(
                  `Request sent to ${reqdialog.getInputValue(0)}`,
                  "OK"
                );
              }
              ...
              switch (response.event) {
                case code.inst.CLASS_NOT_FOUND:
                  return reqdialog.showFieldError(
                    0,
                    "No such classroom exists"
                  );
                case code.inst.CLASS_EXISTS:
                  return reqdialog.showFieldError(
                    0,
                    "Already requested or in the classroom"
                  );
                default:
                  return parent.snackbar(response.event);
              }
...
class ReceiveData {
  constructor() {
    ...
    this.pseudoclasses = this.pseudoclasses.includes("")
      ? []
      : this.pseudoclasses;
  }
}
