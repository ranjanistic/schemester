...
class UIID {
  constructor(back = false) {
    ...
    getElement(
      "subtext"
    ).innerHTML = `Please provide the unique ID of your institution to proceed.`;
    ...
    this.saveuiid = false;
    this.rememberuiid.onTurnChange(
      (_) => {
        this.saveuiid = true;
        this.uiidField.onTextInput((_) => {
          localStorage.setItem(
            key.student.rememberuiid,
    ...
    if (
      localStorage.getItem(key.student.rememberuiid) &&
      localStorage.getItem(key.student.rememberuiid) != "null"
    ) {
      if (!back) {
  ...
  uiidProcedure(uiid) {
    postJsonData(post.student.auth, {
      ...
      .then((response) => {
        this.uiidCheck(
          response.event == code.inst.INSTITUTION_EXISTS ? response.uiid : null
        );
...
class Classname {
  constructor() {
    ...
    getElement(
      "subtext"
    ).innerHTML = `Now, provide your class name which you've enrolled with ${this.getUIID()} institute.`;
    this.classField = new TextInput(
      "userclassfield",
      "Classname",
      "Your base class",
      validType.nonempty
...
class Email {
  constructor() {
    ...
    getElement(
      "subtext"
    ).innerHTML = `Now, provide your email address which you've registered with ${this.getUIID()} institute.`;
...
class Password {
  constructor() {
    ...
    this.target = stringIsValid(this.target, validType.nonempty)
      ? this.target
      : locate.student.target.today;
  ...
  linkSender() {
    if (!stringIsValid(this.getEmail(), validType.email)) {
      this.previous.click();
      return snackBar("Provide your valid email address");
    }
    snackBar(
      `To reset your password, a link will be sent to your provided ${this.getEmail()} address.`,
      "Send Link",
      true,
      (_) => {
        this.passField.forgot.onclick = (_) => {};
