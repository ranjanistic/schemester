...
class StudentDash {
  constructor() {
    ...
    this.fragpath = [
      locate.student.target.fragment.today,
      locate.student.target.fragment.fullweek,
      locate.student.target.fragment.classroom,
      locate.student.target.fragment.settings,
    ];
    ...
    this.tabs.forEach((tab, t) => {
      tab.onclick = (_) => {
        ...
        this.selectTab(tab);
        this.frame.src =
          locate.student.fragment +
          getRequestBody({
            fragment: this.fragpath[t],
            day: new Date().getDay(),
          });
      };
    });
    this.tabs[
      this.fragpath.includes(this.frag)
        ? this.fragpath.indexOf(this.frag)
        : this.fragpath.includes(sessionStorage.getItem(key.fragment))
        ? this.fragpath.indexOf(sessionStorage.getItem(key.fragment))
        : 0
    ].click();
...
async function linkSender(onsuccess = (_) => {}, onsendclick = (_) => {}) {
  snackBar(
    `To reset your password, a link will be sent to your email address.`,
    "Send Link",
    true,
    (_) => {
      onsendclick();
...
function snackbar(
  ...
  snackBar(text, actionText, isNormal, (_) => {
    action();
  });
}
class Pseudostudent {
  constructor() {
    ...
    this.deleteRequest.onclick = (_) => {
      snackBar(
        "Deleting the request will also remove your account on Schemester.",
        "Delete Request",
        false,
        (_) => {
    ...
    this.name = new Editable(
      "studentnameview",
      "studentnameeditor",
      new TextInput("studentnamefield", false, "", validType.name),
      "editstudentname",
      "studentname",
      "savestudentname",
      "cancelstudentname"
