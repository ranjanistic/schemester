...
class Schedule {
  constructor() {
    ...
    this.editmode.onTurnChange(
      (_) => {
        sessionStorage.setItem("switchclash", true);
        this.editmodeview.innerHTML = `Switch Edit`;
        snackBar(
          "Any change will be switched if clashed with someone.",
          "Means?",
          bodyType.warning,
          (_) => {
            infoDialog(
              "Edit modes",
              `The edit mode type tells schemester how to respond if any change made by you results in any kind of conflict.<br/>
                    <ul>
    ...
    tryCalling(
      () => {
        window.fragment = this.data.isTeacher()
          ? new Teacher(this.data)
          : new Class(this.data);
      },
      () => {
        new NoSchedule();
...
class Teacher {
  constructor() {
    ...
    if (!this.data.pending) {
      ...
      this.removeteacher.onclick = (_) => {
        confirmDialog(
          "Remove teacher?",
          `Are you sure you want to remove ${this.data.teachername} (${this.data.teacherID}) from your institution?
                    Their schedule will not be affected, but they won't be
                able to login to your institution. They'll have to join again if needed.`,
          null,
          (_) => {
          ...
          (_) => {
            new Dialog().hide();
            snackBar(`${this.data.teachername}'s account is safe.`);
          }
        );
      };
    }
    this.deleteschedule.onclick = (_) => {
      ...
      confirmdialog.setDisplay(
        "Delete schedule?",
        `Are you sure you want to delete ${
          this.data.pending ? this.data.teacherID : this.data.teachername
        }'s schedule permanently?${
          this.data.pending
            ? " Their invitation will no longer remain valid."
            : " This might affect schedule of several classes too."
        }`
      );
      this.data.pending
        ? confirmdialog.createActions(
            [action.remove, "Abort"],
            [actionType.negative, actionType.neutral]
          )
        : confirmdialog.createActions(
            ["Check affected classes", "Delete anyway", "Abort"],
            [actionType.positive, actionType.negative, actionType.neutral]
          );
      confirmdialog.transparent();
      this.data.pending
        ? confirmdialog.onButtonClick([
            (_) => {
              confirmdialog.loader();
              snackBar(
                `Deleting ${this.data.teacherID}'s schedule...`,
                null,
                false
              );
              ...
              postJsonData(post.admin.receivedata, {
                ...
                .then((resp) => {
                  if (resp.event == code.OK) {
                    ...
                    confirmdialog.setDisplay(
                      "Affected classes",
                      `Deleting ${this.data.teachername}'s schedule will affect following classes.<br/>
                        ${classlist}<div>`
                    );
                    const editclasses = [];
                    resp.classes.forEach((Class, c) => {
                    ...
                    confirmdialog.onButtonClick([
                      (_) => {
                        confirmdialog.loader();
                        snackBar(
                          `Deleting ${this.data.teachername}'s schedule (${this.data.teacherID})...`,
                          null,
                          false
                        );
                        postJsonData(post.admin.schedule, {
    ...
    this.data.weekdays.forEach((dindex, index) => {
      ...
      this.dayshows[index].onclick = (_) => {
        ...
        this.dayshows[index].onclick = (_) => {
          isToday
            ? show(this.periodsView[index])
            : hide(this.periodsView[index]);
          this.dayshows[index].innerHTML = isToday ? "Hide" : "Show"; //replace by rotated icon;
    ...
    this.setDataForDindex(
      Number(
        this.data.weekdays.includes(String(new Date().getDay()))
          ? new Date().getDay()
      ...
      postJsonData(post.admin.schedule, {
        ...
        .then((response) => {
          if (response.event == code.OK) {
            ...
            this.theday = this.schedule.days.find(
              (day) => day.dayIndex == dayIndex
            );
            this.setPeriodsView(
              [this.theday.period][0],
              this.data.weekdays.indexOf(String(dayIndex))
            );
          } else {
            this.schedule == null;
          }
  ...
  setPeriodsView(periods, index) {
    ...
    periods.forEach((period, p) => {
      rows += `<div class="fmt-row break" id="period${index}${p}">
                ...
                <div class="fmt-col fmt-quarter fmt-padding-small positive" id="classnameview${index}${p}">
                    <span id="classname${index}${p}">${
        period.classname
      }</span> <button class="neutral-button caption" id="editclassname${index}${p}"><img width="20" src="/graphic/elements/editicon.svg"/></button>
                </div>
                <div class="fmt-col fmt-quarter fmt-padding-small positive" id="classnameeditor${index}${p}">
                    <fieldset style="margin:0" class="text-field questrial"  id="classnamefield${index}${p}">
                        <legend class="field-caption">Replace ${
                          period.classname
                        }</legend>
                        <input class="text-input" style="font-size:18px" required value="${
                          period.classname
                        }" placeholder="New class" type="text" id="classnameinput${index}${p}" name="classname" >
                ...
                <div class="fmt-col fmt-half fmt-padding-small positive" id="subjectview${index}${p}">
                    <span id="subject${index}${p}">${
        period.subject
      }</span> <button class="neutral-button caption" id="editsubject${index}${p}"><img width="20" src="/graphic/elements/editicon.svg"/></button>
                </div>
                <div class="fmt-col fmt-half fmt-padding-small positive" id="subjecteditor${index}${p}">
                    <fieldset style="margin:0" class="text-field questrial" id="subjectfield${index}${p}">
                        ...
                        <input class="text-input" style="font-size:18px" required value="${
                          period.subject
                        }" placeholder="New subject" type="text" id="subjectinput${index}${p}" name="subject" >
    ...
    periods.forEach((period, p) => {
      ...
      this.classeditable.push(
        new Editable(
          `classnameview${index}${p}`,
          `classnameeditor${index}${p}`,
          new TextInput(
            `classnamefield${index}${p}`,
            `Replace ${period.classname}`,
            `New class`,
            validType.nonempty
          ),
          `editclassname${index}${p}`,
          `classname${index}${p}`,
          `saveclassname${index}${p}`,
          `cancelclassname${index}${p}`,
          `classnameloader${index}${p}`
        )
      );
      this.classeditable[p].onSave((_) => {
        ...
        if (
          this.classeditable[p].getInputValue() ==
          this.classeditable[p].displayText()
        ) {
        ...
        postJsonData(post.admin.schedule, {
          ...
          .then((response) => {
            ...
            if (response.event == code.OK) {
              this.schedule = null;
              this.dayshows[
                this.data.weekdays.indexOf(this.currentdayIndex)
              ].click();
              return;
            }
            switch (response.event) {
              case code.schedule.SCHEDULE_CLASHED: {
                ...
                return (getElement(`clashlink${response.clash.id}`).onclick = (
                  _
                ) => {
                  refer(locate.admin.session, {
              ...
              case code.inst.CLASS_NOT_FOUND: {
                return snackBar(
                  "No such classroom exists. You can create a new class in classrooms view.",
                  "Show classrooms",
                  bodyType.warning,
                  (_) => {
                    refer(locate.admin.session, {
              ...
              default:
                return this.classeditable[p].textInput.showError("Error");
            }
          })
          .catch((err) => {
            snackBar(err, "Report");
      ...
      this.subjecteditable.push(
        new Editable(
          `subjectview${index}${p}`,
          `subjecteditor${index}${p}`,
          new TextInput(
            `subjectfield${index}${p}`,
            `Replace ${period.subject}`,
            `New subject`,
            validType.nonempty
          ),
          `editsubject${index}${p}`,
          `subject${index}${p}`,
          `savesubject${index}${p}`,
          `cancelsubject${index}${p}`,
          `subjectloader${index}${p}`
        )
      );
      this.subjecteditable[p].onSave((_) => {
        ...
        if (
          this.subjecteditable[p].getInputValue() ==
          this.classeditable[p].displayText()
        ) {
        ...
        postJsonData(post.admin.schedule, {
          ...
          .then((response) => {
            ...
            switch (response.event) {
              default:
                return this.subjecteditable[p].textInput.showError("Error");
            }
          })
          .catch((err) => {
            snackBar(err, "Report");
        ...
        if (this.data.weekdays[index] == date.getDay()) {
          ...
          try {
            const day = this.schedule.days.find(
              (day) => day.dayIndex == date.getDay()
            );
            if (
              this.data.start + p * gap <= hrsnow &&
              hrsnow < this.data.start + (p + 1) * gap
            ) {
              //in the schedule duration
              day.period[p].hold
                ? setActive(this.dayperiods[p])
                : setNegativeActive(this.dayperiods[p]);
            }
          } catch {
...
class Class {
  constructor() {
    ...
    this.removeclass.onclick = (_) => {
      confirmDialog(
        "Remove classroom?",
        `Removing ${this.data.classname} will also remove all its students and their accounts in the class, and unset the incharge too.
            Also, the teachers having periods in this class in their schedule, will be set as free on those periods of their schedule.`,
        null,
        (_) => {
          postJsonData(post.admin.users, {
    ...
    this.classname = new Editable(
      "classnameview",
      "classnameeditor",
      new TextInput(
        "classnamefield",
        "New classname",
        `Change ${this.data.classname} to`,
        validType.nonempty
      ),
      "editclassname",
      "classname",
    ...
    this.classname.onSave((_) => {
      if (!this.classname.isValidInput())
        return this.classname.validateInputNow();
      if (
        this.classname.getInputValue().trim() == this.classname.displayText()
      ) {
      ...
      postJsonData(post.admin.schedule, {
        ...
        if (resp.event == code.OK) {
          if (resp.msg == code.inst.CLASS_EXISTS) {
            snackBar(
              `Class ${this.classname.displayText()} has been replaced with ${this.classname.getInputValue()}, and the latter one is the former now.`,
              "Undo",
              true,
              (_) => {
            ...
            snackBar(
              `Class ${this.classname.displayText()} is renamed as ${this.classname.getInputValue()}.`,
              "Undo",
              true,
              (_) => {
                this.classname.saveButton.click();
        ...
        switch (resp.event) {
          case code.inst.CLASS_EXISTS:
            return this.classname.textInput.showError("Class already exists");
          case code.schedule.SCHEDULE_CLASHED:
            return this.classname.textInput.showError("Clashed");
        }
      });
    });
  ...
  setPeriodsView(periods, index) {
      ...
      rows += `<br/>
                <div class="fmt-row break" id="period${index}${p}">
                    ...
                    <div class="fmt-col fmt-quarter fmt-padding-small positive" id="subjectview${index}${p}">
                        <span id="subject${index}${p}">${
        period ? period.subject : "Not set"
      }</span> <button class="neutral-button caption" id="editsubject${index}${p}">${editIcon(
        15
      )}</button>
                    </div>
                    ...
                    <div class="fmt-col fmt-half fmt-padding-small positive" id="teacherIDview${index}${p}">
                        <span class="" id="teachername${index}${p}">${
        period ? period.teachername : "Not set"
      }</span><button class="neutral-button caption" id="editteacherID${index}${p}">${editIcon(
        15
      )}</button>
                        <br/>
                        <span class="group-text" id="teacherID${index}${p}">${
        period ? period.teacherID : "Not set"
      }</span>
                    </div>
                    <div class="fmt-col fmt-half fmt-padding-small positive" id="teacherIDeditor${index}${p}">
                        <fieldset style="margin:0" class="text-field questrial fmt-third" id="teacherIDfield${index}${p}">
                            ...
                            <input class="text-input" style="font-size:18px" required value="${
                              period ? period.teacherID : ""
                            }" placeholder="New teacherID" type="text" id="teacherIDinput${index}${p}" name="teacherID" >
    ...
    periods.forEach((period, p) => {
      ...
      this.subjecteditable[p].onSave((_) => {
        ...
        if (
          this.subjecteditable[p].getInputValue() ==
          this.subjecteditable[p].displayText()
        ) {
          return this.subjecteditable[p].clickCancel();
        }
      ...
      this.teachereditable.push(
        new Editable(
          `teacherIDview${index}${p}`,
          `teacherIDeditor${index}${p}`,
          new TextInput(
            `teacherIDfield${index}${p}`,
            `Replace ${period ? period.teachername : ""}`,
            `New teacher ID`,
            validType.email
          ),
          `editteacherID${index}${p}`,
          `teacherID${index}${p}`,
          `saveteacherID${index}${p}`,
          `cancelteacherID${index}${p}`,
          `teacherIDloader${index}${p}`
        )
      );
      this.teachereditable[p].textInput.onTextInput((_) => {
        this.teacherpredictor(
          this.teachereditable[p].getInputValue().trim(),
          this.teachereditable[p].textInput
        );
      });
      this.teachereditable[p].onSave((_) => {
        ...
        if (
          this.teachereditable[p].getInputValue().trim() ==
          this.teachereditable[p].displayText()
        ) {
          ...
          .then((response) => {
            ...
            switch (response.event) {
              case code.auth.USER_NOT_EXIST:
                return this.teachereditable[p].textInput.showError(
                  "No such teacher"
                );
              default:
                return this.teachereditable[p].textInput.showError("Error");
            }
          })
          .catch((err) => {
            snackBar(err, "Report");
...
class ReceiveData {
  constructor() {
    ...
    if (this.isTeacher()) {
      this.teacherID =
        getElement("teacherID").innerHTML == "null"
          ? null
          : getElement("teacherID").innerHTML;
      if (!this.pending) {
        this.teacherUID = getElement("teacherUID").innerHTML;
        this.teachername = getElement("teachername").innerHTML;
        this.verified =
          getElement("teacherverified").innerHTML == "true" ? true : false;
      }
    } else {
      this.classname =
        getElement("classname").innerHTML == "null"
          ? null
          : getElement("classname").innerHTML;
      if (!this.pending) {
