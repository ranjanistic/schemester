        ...
        postJsonData(post.admin.dashboard, {
          ...
          .then((response) => {
            ...
            if (response.timings.daysInWeek.includes(new Date().getDay())) {
              ...
              this.todaywork.innerHTML = `<div class="fmt-row" id="todayclassesview" >
            ...
            `;
              ...
              if (!affectedclasses.length) {
                ...
                affectedclasses.forEach((period, p) => {
                  classeslist += `
                  <div class="fmt-row neutral-button" style="margin:4px 0">
                    ${addNumberSuffixHTML(period.p + 1)} period of ${
                    period.classname
                  } is vacant.
                ...
                affectedclasses.forEach((_, p) => {
                  ...
                  arrangebuttons[p].onclick = (_) => {
                    ...
                    adialog.setDisplay(
                      "",
                      `Substitute for ${addNumberSuffixHTML(
                        arranges[p].period + 1
                      )} period of class ${arranges[p].classname}`
                    );
                    adialog.createInputs(
                      ["Substitute name or ID"],
                      ["This teacher will hold the class today"],
                      ["text"],
                      [validType.nonempty]
                    );
                    adialog.createActions(
                      ["Set", "Cancel"],
                      [actionType.positive, actionType.neutral]
                    );
                    adialog.transparent();
                    adialog.onButtonClick([
                      (_) => {
                        ...
                        postJsonData(post.admin.dashboard, {
                          ...
                          switch (resp.event) {
                            case code.auth.USER_NOT_EXIST:
                              return adialog.inputField[0].showError(
                                "No such teacher",
                                true
                              );
                            default:
                ...
                getElement("autoarrange").onclick = (_) => {
                  ...
                  autodialog.setDisplay(
                    "Auto arrange?",
                    "Are you sure you want schemester to arrange substitutes for you? This feature is experimental, and might fail to arrange."
                  );
                  autodialog.createActions(
                    ["Go ahead", "Abort"],
                    [actionType.positive, actionType.neutral]
                  );
                  autodialog.onButtonClick([
                    (_) => {
                      autodialog.loader();
                      snackBar("Please wait");
                      autodialog.setDisplay(
                        "Arranging...",
                        "Finding and assigning substitutes for today..."
                      );
                      //autoarrange;
                    },
                    (_) => {
    ...
    tryCalling(() => {
      getElement("institutesetup").onclick = (_) => {
        refer(locate.admin.session, {
          ...
          section: locate.admin.section.institute,
        });
      };
    });
...
class ConfirmClasses {
  constructor(receivedclasses) {
    ...
    class InchargeDialog {
      constructor(receivedclasses) {
        ...
        receivedclasses.forEach((Class, c) => {
          ...
          this.inchargeeditables[c].textInput.onTextInput((_) => {
            this.teacherpredictor(c);
          });
          this.inchargeeditables[c].onSave((_) => {
            ...
            this.inchargeeditables[c].setDisplayText(`Class ${Class} 
              <span class="positive caption">${sessionStorage.getItem(
                Class + "name"
              )} (${sessionStorage.getItem(Class)})</span>`);
      ...
      teacherpredictor = (c) => {
        if (
          this.inchargeeditables[c].getInputValue() &&
          this.inchargeeditables[c].getInputValue().trim() != "@" &&
          this.inchargeeditables[c].getInputValue().trim() != "." &&
          this.inchargeeditables[c].getInputValue().trim() != constant.nothing
        ) {
            ...
            if (resp.event == code.OK) {
              if (resp.teachers.length > 0) {
                snackBar(
                  ...
                  true,
                  (_) => {
                    this.inchargeeditables[c].textInput.setInput(
                      resp.teachers[0].teacherID
                    );
                    sessionStorage.setItem(
                      this.receivedclasses[c],
                      resp.teachers[0].teacherID
                    );
                    sessionStorage.setItem(
                      this.receivedclasses[c] + "name",
                      resp.teachers[0].username
                    );
                  }
                );
              }
    ...
    class RenameDialog {
      constructor(receivedclasses) {
        this.receivedclasses = receivedclasses;
        let bodyview = receivedclasses.length
          ? `<center>${receivedclasses.length} unique classes found. 
          Rename classes, or edit the duplicate classes and rename them as actual ones, then continue.</center>
          <br/>
          <div class="fmt-col">`
          : `No schedule is available of any teacher.`;
        receivedclasses.forEach((Class, c) => {
          //each class row
          bodyview += `
        <div class="fmt-row fmt-padding" id="classrow${c}">
          ...
          this.classeditables[c].onSave((_) => {
            ...
            postJsonData(post.admin.schedule, {
              ...
              if (response.event == code.OK) {
                ...
                this.classeditables[c].textInput.setFieldCaption(
                  `Rename ${this.classeditables[c].getInputValue().trim()} as`
                );
              ...
              switch (response.event) {
                case code.schedule.SCHEDULE_CLASHED:
                  return this.classeditables[c].textInput.showError(
                    `This change will clash period ${
                      response.clash.targetperiod + 1
                    } on ${constant.weekdays[response.clash.targetday]} of
                    <a target="_blank" rel="noreferrer" href="${
                      locate.admin.session
                    }?target=${locate.admin.target.viewschedule}&type=${
                      client.teacher
                    }&teacherID=${response.clash.targetid}" id="changer${
                      response.clash.targetid
                    }">${response.clash.targetid}</a>
                    with <a target="_blank" rel="noreferrer" href="${
                      locate.admin.session
                    }?target=${locate.admin.target.viewschedule}&type=${
                      client.teacher
                    }&teacherID=${response.clash.id}" id="clash${
                      response.clash.targetid
                    }">${response.clash.id}</a>`
                  );
                default:
...
class BaseView {
  constructor() {
    ...
    try {
      getElement("resumeschedule").onclick = (_) => {
        this.instname.click();
      };
    } catch {}
  }
