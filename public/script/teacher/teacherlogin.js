...
class UIID {
  constructor(back = false) {
    ...
    if (
      localStorage.getItem(key.teacher.rememberuiid) &&
      localStorage.getItem(key.teacher.rememberuiid) != "null"
    ) {
      if (!back) {
  ...
  uiidCheck(checkeduiid = null) {
    checkeduiid
      ...
      : sessionStorage.removeItem(key.uiid);
  }
  loader(show = true) {
    visibilityOf(this.logInLoader, show);
    visibilityOf(this.proceed, !show);
