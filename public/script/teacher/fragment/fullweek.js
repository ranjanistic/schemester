...
if (
  sessionStorage.getItem("fragment") != locate.teacher.target.fragment.fullweek
) {
  parent.clickTab(1);
}
class FullSchedule {
  constructor() {
    ...
    if (this.data.schedule) {
      ...
      getElement("seesettings").onclick = window.parent.document.getElementById(
        "settingstab"
      ).onclick;
    }
    this.startTimers();
  ...
  controlVisibility() {
    ...
    this.data.totaldays.forEach((dindex, d) => {
      new Date().getDay() == this.data.totaldays[d]
        ? this.setActive(this.dayBox[d])
        : (_) => {};
...
class ReceiveData {
  constructor(raw = false) {
    ...
    if (this.timings) {
      ...
      this.schedule =
        getElement("schedule").innerHTML &&
        getElement("schedule").innerHTML != "false"
          ? true
          : false;
