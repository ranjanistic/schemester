...
if (
  ...
  locate.teacher.target.fragment.classroom
) {
  parent.clickTab(2);
}
class Classroom {
  constructor() {
    ...
    this.chooseclass.onclick = (_) => {
      ...
      this.data.otherclasses.forEach((Class, c) => {
        ...
        <div class="fmt-row fmt-center">
          <button class="half ${
            Class == sessionStorage.getItem("inchargeof")
              ? "active-button"
              : "positive-button"
          }" id="classbutton${c}">${
          Class != sessionStorage.getItem("inchargeof")
      ...
      classchoose.setDisplay(
        "Choose class",
        `These are the classes you take in your schedule, choose anyone to view.<br/>${viewbody}`
      );
      let classbtns = [];
      this.data.otherclasses.forEach((Class, c) => {
    ...
    this.students.forEach((stud, s) => {
      stud.removestudent.onclick = (_) => {
        ...
        removeconfirm.setDisplay(
          "Remove student?",
          `Are you sure you want to remove <span class="negative">${stud.studentName}</span> (${stud.studentID})?
        ...
        removeconfirm.createActions(
          [`Remove ${stud.studentName}`, "No, abort"],
            ...
            postJsonData(post.teacher.classroom, {
              ...
              .then((response) => {
                if (response.event == code.OK) {
                  removeconfirm.hide();
                  parent.snackbar(
                    `${stud.studentName} was removed from ${this.data.classname}`
                  );
              ...
              .catch((e) => {
                clog(e);
                parent.snackbar(
                  `${stud.studentName} was not removed.`,
                  "Retry",
                  false,
                  (_) => {
              ...
              accepts[t].onclick = (_) => {
                ...
                postJsonData(post.teacher.classroom, {
                  ...
                  if (resp.event == code.OK) {
                    hide(getElement(`request${t}`));
                    parent.snackbar(
                      `Accepted ${student.username} (${student.studentID})`
                    );
                  }
                  requestDialog.loader(false);
                });
      ...
      .then((response) => {
        if (
          ...
          linkdialog.setDisplay(
            "Invitation Link",
            `<center><a target="_blank" rel="noreferrer" href="${
              response.link
            }">${response.link}</a>
        ...
        switch (response.event) {
          case code.invite.LINK_EXISTS:
            return parent.snackbar(
              "This link already exists and can be shared."
            );
