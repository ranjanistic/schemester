/**
 * For global date and time management methods.
...
class Time {
  ...
  getMoment(
    ...
    year += increment.year;
    return this.getTemplateNotation(
      year,
      month,
      day,
      hour,
  ...
  getTemplateNotation(
    ...
    return string
      ...
      : Number(`${year}${month}${day}${hour}${minute}${seconds}${millis}`);
  }
  ...
  getTheMoment = (stringForm = true, dayincrement = 0) =>
    this.getMoment({ day: dayincrement }, stringForm);
   ...
   */
  getTheMomentMinute = (minuteIncrement = 0, stringform = true) =>
    this.getMoment({ minute: minuteIncrement }, stringform);
}
