...
class Session {
  ...
  createTempSession(response, userID, userUIID, clientType) {
    ...
    const token = jwt.sign(payload, getSecretByClient(clientType), {
      expiresIn: this.expiresInTemp,
    });
    try {
      return response.cookie(this.sessionKey, token, {
        signed: true,
        expires: new Date(Date.now() + this.expiresInTemp * 1000),
        httpOnly: true,
        secure: true,
        sameSite: "Lax",
      });
  ...
  async login(request, response, clientType, with2fa = true) {
    ...
    switch (clientType) {
      case client.admin: {
        ...
        if (inst) {
          const isAdmin = await adminworker.inst.isAdminOfInstitute(
            uiid,
            email
          );
          if (!isAdmin) {
      ...
      case client.teacher:
        {
          ...
          switch (body.type) {
            ...
            default: {
              ...
              let teacher = await teacherworker.self.account.getTeacherByEmail(
                body.uiid,
                body.email
              );
              const pteacher = await teacherworker.self.account.getTeacherByEmail(
                body.uiid,
                body.email,
                true
              );
              switch (body.type) {
                ...
                case this.password: {
                  ...
                  teacher = teacher ? teacher : pteacher;
            ...
            default: {
              ...
              switch (body.type) {
                ...
                case this.email: {
                  ...
                  const pseudo = pseudoclassdoc.pseudousers.classes[0].students.find(
                    (stud) => stud.studentID == body.email
                  )
                    ? true
                    : false;
                  const isintheclass = classdoc.users.classes[0].students.find(
                    (stud) => stud.studentID == body.email
                  )
                    ? true
                    : false || pseudo;
                  const isinanyclass = await studentworker.classes.getClassesByStudentID(
                    body.uiid,
                    body.email
                  );
                  try {
                    if (!isinanyclass) {
                      //then delete the student's account
                      let account = await studentworker.self.account.getStudentByEmail(
                        body.uiid,
                        body.email,
                        false
                      );
                      account = account
                        ? account
                        : await studentworker.self.account.getStudentByEmail(
                            body.uiid,
                            body.email,
                            true
                          );
                ...
                case this.password:
                  {
                    ...
                    const pseudo = pseudoclassdoc.pseudousers.classes[0].students.find(
                      ...
                      ? true
                      : false;
                    const student = await studentworker.self.account.getStudentByEmail(
                      uiid,
                      email,
                      pseudo
                    );
  ...
  async authenticate(req, res, body, clientType) {
    ...
    switch (clientType) {
      ...
      case client.student: {
        const student = await studentworker.self.account.getAccount(
          resp.user,
          true
        );
  ...
  async signup(request, response, clientType, pseudo = false) {
    switch (clientType) {
      case client.admin: {
        ...
        if (admin) {
          if (request.body.isinvite) {
            //accepting invitation join
            const inst = await Institute.findOne({ [this.uiid]: uiid });
            if (!inst) return code.event(code.inst.INSTITUTION_NOT_EXISTS);
            if (!inst.invite.admin.active)
              return code.event(code.invite.LINK_INVALID);
            if (inst.default.admin.find((ad) => ad.email == email))
              return code.event(code.auth.USER_EXIST);
            const ismatch = await bcrypt.compare(password, admin.password);
            if (!ismatch) return code.event(code.auth.WRONG_PASSWORD);
            const joined = await adminworker.inst.joinInstituteAsAdmin(
              inst._id,
              admin
            );
      ...
      case client.teacher:
        {
          ...
          if (result.event == code.NO)
            return code.event(code.auth.ACCOUNT_CREATION_FAILED);
          teacher = await teacherworker.self.account.getTeacherByEmail(
            uiid,
            email,
            pseudo
          );
      ...
      case client.student: {
        ...
        const inst = await Institute.findOne({ [this.uiid]: uiid });
        if (!inst) return code.event(code.inst.INSTITUTION_NOT_EXISTS);
        const classroom = await studentworker.classes.getClassByClassname(
          uiid,
          classname
        );
        if (!classroom) return code.event(code.auth.CLASS_NOT_EXIST); //if classroom not in users
        if (classroom.students.find((stud) => stud.studentID == email))
          return code.event(code.auth.USER_EXIST); //if student in the classroom
        const pclassroom = await studentworker.classes.getClassByClassname(
          uiid,
          classname,
          true
        );
        if (!pclassroom) return code.event(code.auth.CLASS_NOT_EXIST); //if classroom not in pseudo
        if (pclassroom.students.find((stud) => stud.studentID == email))
          return code.event(code.auth.USER_EXIST); //if student in pseudo classroom
        let student = await studentworker.self.account.getStudentByEmail(
          uiid,
          email
        );
        if (student) {
          //if student account exists in institute, add student to the classroom (according to pseudo)
          const isMatch = await bcrypt.compare(password, student.password);
          if (!isMatch) return code.event(code.auth.WRONG_PASSWORD);
          return await studentworker.self.account.addStudentToClass(
            uiid,
            classname,
        ...
        let pstudent = await studentworker.self.account.getStudentByEmail(
          uiid,
          email,
          true
        );
        if (pstudent) {
          //if pseudo student account exists in institute, add student to the pseudo classroom
          const isMatch = await bcrypt.compare(password, student.password);
          if (!isMatch) return code.event(code.auth.WRONG_PASSWORD);
          return await studentworker.self.account.addStudentToClass(
            uiid,
            classname,
            { username: pstudent.username, studentID: pstudent.studentID }
          );
        }
        ...
        let result = await studentworker.self.account.createAccount(
          uiid,
          newstudent,
          pseudo
        ...
        result = await studentworker.self.account.addStudentToClass(
          uiid,
          classname,
          { username: newstudent.username, studentID: newstudent.studentID },
          pseudo
        );
        if (result.event == code.NO)
          return code.event(code.inst.CLASS_JOIN_FAILED);
        this.createSession(
          response,
          newstudent._id,
          uiid,
        ...
        return {
          event: code.auth.ACCOUNT_CREATED,
          user: pseudo
            ? share.getPseudoStudentShareData(newstudent)
            : share.getStudentShareData(newstudent),
        };
      }
      default:
  ...
  valid = (response, clientType) =>
    response.event != code.auth.SESSION_INVALID &&
    inspect.sessionTokenValid(response.user, clientType);
}

const getSecretByClient = (clientType) => {
  switch (clientType) {
    case client.admin:
      return inspect.token.verify(session.adminkey);
    case client.teacher:
      return inspect.token.verify(session.teacherkey);
    case client.student:
      return inspect.token.verify(session.studentkey);
  }
};
