  ...
  constructor() {
    ...
    class Account {
      ...
      async getAccount(user, raw = false) {
        ...
        return raw
          ? student
          : share.getPseudoStudentShareData(student, user.uiid);
      }
      ...
      async addStudentToClass(uiid, classname, student, pseudo = true) {
        const doc = await Institute.findOneAndUpdate(
          {
            uiid: uiid,
            [pseudo ? pseudoclasspath : classpath]: {
      ...
      changeName = async (user, body) => {
        ...
        student = student
          ? student
          : await this.getStudentById(user.uiid, user.id, true);
        ...
        const accdoc = await Institute.findOneAndUpdate(
          {
            uiid: user.uiid,
            [pseudo ? pseudostudentspath : studentspath]: {
        ...
        const classroom = await new Classroom().getClassesByStudentID(
          user.uiid,
          student.studentID,
          true
        );
          ...
          classroom.pseudoclasses.map((pclass) => {
            return new Promise(async (resolve) => {
              const doc = await Institute.findOneAndUpdate(
                ...
                {
                  arrayFilters: [
                    { [`theclass.${this.classname}`]: pclass },
                    { [`thestudent.${this.studentID}`]: student.studentID },
                  ],
                }
              );
              resolve(doc);
      ...
      deleteAccount = async (user) => {
          ...
          classroom.pseudoclasses.map((pclass) => {
            return new Promise(async (resolve) => {
              const doc = await Institute.findOneAndUpdate(
                ...
                {
                  $pull: {
                    [`${pseudoclasspath}.$[theclass].students`]: {
        ...
        const singlerooms = await new Comms().get1to1RoomsofPerson(
          user.uiid,
          student.studentID
        );
        if (singlerooms)
            ...
            list.rooms.map((room) => {
              return new Promise(async (resolve) => {
                const doc = await Institute.findOneAndUpdate(
                  ...
                  {
                    arrayFilters: [
                      { [`theroom.${this.uid}`]: ObjectId(room._id) },
                    ],
                  }
                );
                resolve(doc);
    ...
    switch (body.action) {
      case action.send: {
        ...
        if (!user) {
          ...
          //user not logged in
          let student = await this.account.getStudentByEmail(
            body.uiid,
            body.email
          );
          if (!student) {
            student = await this.account.getStudentByEmail(
              body.uiid,
              body.email,
              true
            );
          ...
          return await this.handlePassReset(
            {
              id: share.getPseudoStudentShareData(student).uid,
              uiid: body.uiid,
            },
            body
          );
      ...
      inst.default.admin.map((admin, a) => {
        return new Promise(async (resolve) => {
          ...
          inst.default.admin[a]["phonevisible"] =
            useradmin.prefs.showphonetostudent;
          inst.default.admin[a]["emailvisible"] =
            useradmin.prefs.showemailtostudent;
          resolve(inst);
        });
      })
    );
...
class Schedule {
  ...
  async getSchedule(user, dayIndex = null) {
    ...
    if (dayIndex != null) {
      ...
      return {
        schedule: thisday.hasOwnProperty("period") ? thisday : false,
        timings: timings,
      };
    } else {
      teacherdoc.schedule.teachers.some((teacher) => {
      ...
      //   })
      // })
      // days[0].period[0]==undefined?true:false
...
class Classroom {
  ...
  async getClassesByStudentID(uiid, studentID, classnameonly = false) {
    ...
    cdoc.users.classes.forEach((Class) => {
      if (
        Class.students.find((stud) => stud.studentID == studentID)
          ? true
          : false
      ) {
        classes.push(Class);
  ...
  async getClassByClassname(uiid, classname, pseudo = false) {
    const doc = await Institute.findOne(
      {
        uiid: uiid,
        [pseudo ? this.account.pseudoclasspath : this.account.classpath]: {
  ...
  async removeStudentFromClass(uiid, classname, studentID, pseudo = true) {
    const doc = await Institute.findOneAndUpdate(
      ...
      },
      {
        $pull: {
          [pseudo
            ? this.account.pseudostudentsclasspath
            : this.account.studentsclasspath]: {
  ...
  async joinPseudoClass(user, classname) {
    ...
    const pseudoclassdoc = await Institute.findOne({
      uiid: user.uiid,
      [this.account.pseudoclasspath]: {
        $elemMatch: { [this.account.classname]: classname },
      },
    });
    if (!pseudoclassdoc) return code.event(code.inst.CLASS_NOT_FOUND);
    const classrooms = await this.getClassesByStudentID(
      user.uiid,
      student.id,
      true
    );
    if (classrooms) {
      if (
        classrooms.classes.includes(classname) ||
        classrooms.pseudoclasses.includes(classname)
      )
        //already in the request class.
        return code.event(code.inst.CLASS_EXISTS);
    }
    return await this.account.addStudentToClass(user.uiid, classname, {
      username: student.username,
  ...
  async handleClassRequest(user, body) {
    switch (body.specific) {
      case "join":
        return await this.joinPseudoClass(user, body.classname);
      case "withdraw":
        return await this.removePseudoRequest(user, body.classname);
    }
  }
}
...
class Comms {
  ...
  async getRoomAndCallList(user) {
    ...
    const classroom = await this.classes.getClassesByStudentID(
      user.uiid,
      student.id
    );
    if (classroom) {
      classroom.classes.forEach((Class) => {
        if (
          !rooms.find((room) => String(room[this.id]) == String(Class[this.id]))
        ) {
          if (
            inst.comms.find(
              (room) => String(room[this.id]) == String(Class[this.id])
            )
              ? true
              : false
          ) {
  ...
  async getRoom(user, roomdata) {
    ...
    if (roomdata.rid) room = await this.getRoomByID(user.uiid, roomdata.rid);
      ...
      if (room) {
        if (!room.people.find((person) => person.id == student.id)) {
          //checking if student is in the room, pushing if not
          const classrooms = await this.classes.getClassesByStudentID(
            user.uiid,
            student.id,
            true
          );
      ...
      if (!room) {
        const person = await this.account.this.getStudentById(
          user.uiid,
          roomdata.personid
        );
    ...
    if (room.blocked.includes(student.id))
